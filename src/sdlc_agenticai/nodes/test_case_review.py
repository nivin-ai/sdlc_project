...
class TestCaseReview:
    ...
    def test_case_review(self, state: State)->dict:
        ...
        test_case_review =  self.llm.invoke(f"Perform a basic review on the following test cases: {state['test_cases']}. If the review is approved, ONLY RETURN 'Approved', and NOTHING ELSE, NO FEEDBACK OR SUGGESTIONS. Else, return 'Failed' with feedback on how to improve the test cases.")
    ...
    def decide_next(self, state: State):
        if "Approved" in state["test_cases_review"]:
