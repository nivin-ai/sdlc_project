...
class DesignDocuments:
    ...
    def generate_design_documents(self, state: State)->dict:
        ...
        design_documents =  self.llm.invoke(f"Create really good, top notch, both functional and technical design documents attending to every detail in the solution, based on the following. Requirement: {state['user_requirement']}. User story: {state['user_story']}.")
