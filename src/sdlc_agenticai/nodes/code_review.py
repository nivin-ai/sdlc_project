...
class CodeReview:
    ...
    def generate_code_review(self, state: State)->dict:
        ...
        code_review =  self.llm.invoke(f"Perform a code review on the following code: {state['generated_code']}. Do not perform security review at this stage. Refer User requirement: {state['user_requirement']}, User Story: {state['user_story']}. If the code document is approved, only return 'Approved' and NOTHING ELSE, DO NOT give any suggestions or Feedback. Else, return 'Failed' with feedback as to how to improve. Fail only if it is really bad.")
